...
pub fn generate_moves(board: &mut engine::Board, movelist: &mut Vec<u16>) -> usize {
    ...
    //king moves
    let king_raw = refresh(board);
    let mut king_captures = king_raw & board.occupancies[raw_enemy];
    let mut king_quiet = king_raw & !king_captures;
